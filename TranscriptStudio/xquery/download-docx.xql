	...
	if (exists($sessionPath)) then
		...
		return
			if (exists($sessionId)) then
				let $result := $utils:dataCollection/session[@id = $sessionId]
				return
					if (not(exists($result))) then
		...
		if (not(exists($sessionId))) then
			...
			return
				if (not(exists($eventId))) then
					...
					let $wordML := utils:transform($doc, 'session-wordml.xsl', <parameters><param name='eventPath' value='{$eventPath}'/></parameters>)
					return
