...
Mark Carter - changed to allow special handling of when TextInput's text field is changed by this manager
              (see itemSelectedFunction())
*/
...
public class AutoCompleteManager extends EventDispatcher
{
	...
	public var itemSelectedFunction:Function = function(text:String, textInput:TextInput):void {
		if (text == null) {
			textInput.text = "";
		}
		else {
     ...
     * @param textInput The TextInput instance
     */
    public function display(show:Boolean, textInput:TextInput):void
    { 
	            ...
	            else 
	            {
	 				...
	 				if(autoFillEnabled && !_removeSelection)
					{
						...
						var lbl:String = itemToLabel(_currentCollection.getItemAt(0));
					   	itemSelectedFunction(lbl, textInput);
					}
	    		...
	    		if(!textInput.length < minCharsForCompletion && _currentCollection !=null)
				{
		         	...
		         	if(!_currentCollection.length < 1)
		         	{
		         		if(textInput.text != itemToLabel(dropdown.selectedItem))
		         		{
		         			...
		         			itemSelectedFunction(itemToLabel( _currentCollection.getItemAt(0) ), textInput);
		         		}
		         	}
		    	}
    ...
    protected function dropdown_itemClickHandler(event:ListEvent):void
    {
        ...
        var dropdown:ListBase = event.currentTarget as ListBase;
    	var textInput:TextInput = dropdown.owner as TextInput;
        itemSelectedFunction(itemToLabel(dropdown.selectedItem), textInput);
        close(event);  
    }
    ...
    protected function keyDownHandler(event:KeyboardEvent):void
    {
        ...
        if (event.keyCode == Keyboard.ESCAPE)
        {
            _inKeyDown = false;
            display(false, textInput);
            itemSelectedFunction(null, textInput);
